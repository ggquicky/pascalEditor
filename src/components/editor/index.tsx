...
import { initSpaceDetectionSync, initSpatialGridSync, useScene } from '@/core'
...
export default function Editor({ projectId, token, baseUrl }: EditorProps) {
  useKeyboard()
  ...
  useEffect(() => {
    if (baseUrl) {
      setApiConfig({ token: token || null, baseUrl })
    }
  }, [token, baseUrl])
